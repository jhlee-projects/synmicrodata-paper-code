#---------------------------------------------------------------------#
# 
# Generate Synthetic Data : complete-case-only for simulation 2
# 
#---------------------------------------------------------------------#
...
if(!dir.exists("03 Output\\Synthetic datasets\\with missing(cc only)")) dir.create("03 Output\\Synthetic datasets\\with missing(cc only)")
...
for(seedNum in 1:sample_num){
  ## complete case only dataset...
  miss_data <- True_data[sample_index[,seedNum],]
  miss_data[miss_index[,,seedNum]==1] <- NA
  
  comp_data <- na.omit(miss_data)
  
  ## prepare multipleSyn
  data_obj = readData(Y_input=comp_data[,y_var], 
                      X_input=comp_data[,x_var], RandomSeed=seedNum)
  
  model_obj = createModel(data_obj, max_R_S_K=c(20,40,20))
  
  syn_comp[[seedNum]] = multipleSyn(data_obj, model_obj, n_burnin, m_num, interval_btw_syn, show_iter=FALSE)
  message(paste0("Sample ", seedNum, " End: ", Sys.time()))
  
  save(syn_comp,
       file=paste0("03 Output\\Synthetic datasets\\with missing(cc only)\\synthetic results (size ", sample_size, ").rda"))
}
